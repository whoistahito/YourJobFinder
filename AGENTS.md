...
- DB: `db_host`, `db_port`, `db_name`, `db_username`, `db_password` (`credential.get_db_uri()` builds the URI). In prod `db_host=ssh-tunnel` — the app connects to the **autossh sidecar** (`docker-compose.yml`), which forwards to the remote Postgres. The app has no SSH code; the sidecar's tunnel is configured via `SSH_HOST`, `SSH_USER`, `SSH_PORT`, `SSH_DB_HOST`, `SSH_DB_PORT`, and `SSH_KEY_B64` (base64 of the OpenSSH private key — Coolify can't manage volume mounts for compose apps, so the key is injected as env and written to `/id_rsa` at startup).
